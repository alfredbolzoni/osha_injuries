...
def sb_select_all(table: str, select: str = "*", filters: dict | None = None, page_size: int = 2000, max_pages: int = 1000) -> pd.DataFrame:
    """
    Download all rows via Supabase REST (paginated with Range headers).
    """
...
with tab1:
    ...
    if df_inc.empty:
        ...
        if len(grp) >= 1:
            ...
            c1.metric("💥 TRIR", f"{latest['TRIR']:.2f}", f"Δ vs {int(prev['year'])}: {delta('TRIR')}" if prev is not None else "")
            c2.metric("📆 Severity Rate", f"{latest['SeverityRate']:.2f}", f"Δ vs {int(prev['year'])}: {delta('SeverityRate')}" if prev is not None else "")
            c3.metric("☠️ Fatality Rate", f"{latest['FatalityRate']:.2f}", f"Δ vs {int(prev['year'])}: {delta('FatalityRate')}" if prev is not None else "")
        ...
        years = sorted(df_inc["year"].dropna().astype(int).unique().tolist())
    ...
    if df_states.empty:
        ...
        if not df_table.empty:
            ...
            st.dataframe(df_table.rename(columns={
                "year":"Year","injuries":"Injuries","fatalities":"Fatalities",
                "daysawayfromwork":"Lost Days (DAFW)","jobtransferrestriction":"Work Restrictions (DJTR)"
...
with tab3:
    ...
    if df_sectors.empty or df_inc.empty:
        st.error("⚠️ Missing sectors/incidents.")
    else:
        ...
        if not df_full.empty and "naics_code" in df_full.columns:
            ...
            top_sub = df_sub.groupby("naics3", dropna=True)["injuries"].sum().reset_index().sort_values("injuries",ascending=False).head(10)
        ...
        if not df_rate.empty:
            rate = df_rate.groupby("sector_macro", dropna=True).agg({"injuries":"sum","employees":"sum"}).reset_index()
            ...
            if not rate.empty:
                st.subheader("⚖️ Injury Rate by Macro Sector (/1000 employees)")
...
with tab4:
    ...
    if df_all.empty:
        ...
        state_sel = st.selectbox("🗺️ Select State:", states, index=0)
        sector_sel = st.selectbox("🏭 Select Macro Sector:", macros, index=0)

        df_f = df_all[(df_all["year"]==year_sel)&(df_all["state_name"]==state_sel)&(df_all["sector_macro"]==sector_sel)]
        df_tbl = df_f.agg({"injuries":"sum","fatalities":"sum","hoursworked":"sum","employees":"sum"}).to_frame().T
        ...
        if not df_tbl.empty:
            ...
            st.dataframe(pd.DataFrame({
                "Year":[year_sel],"State":[state_sel],"Macro Sector":[sector_sel],
                "Injuries":[int(df_tbl.loc[0,"injuries"])],"Fatalities":[int(df_tbl.loc[0,"fatalities"])],
                "TRIR (/200k hrs)":[df_tbl.loc[0,"TRIR (/200k hrs)"]],
            }), use_container_width=True)

            df_tr = df_all[(df_all["state_name"]==state_sel)&(df_all["sector_macro"]==sector_sel)].groupby("year",dropna=True)["injuries"].sum().reset_index()
            if not df_tr.empty:
            ...
            with cA: delta_emp = st.slider("Change in Employees (%)",-30,30,0,step=5)
            with cB: delta_hours = st.slider("Change in Hours Worked (%)",-30,30,0,step=5)
            with cC: delta_inj = st.slider("Change in Injuries (%)",-50,50,0,step=5)
...
with tab5:
    ...
    df_all = incidents_with_state_sector()
    if df_all.empty:
        st.error("⚠️ df_all is empty.")
    else:
        latest_year = int(df_all["year"].dropna().max())
        df_y = df_all[df_all["year"]==latest_year]

        states_rate = df_y.groupby("state_name",dropna=True).agg({"injuries":"sum","hoursworked":"sum"}).reset_index()
        ...
        sectors_rate = df_y.groupby("sector_macro",dropna=True).agg({"injuries":"sum","hoursworked":"sum"}).reset_index()
        ...
        c1,c2,c3 = st.columns(3)
        year_opt = [None]+sorted(df_all["year"].dropna().astype(int).unique().tolist())
        with c1: exp_year = st.selectbox("Year (optional)", year_opt,index=0)
        with c2: exp_state = st.selectbox("State (optional)", [None]+states,index=0)
        with c3: exp_sector = st.selectbox("Macro Sector (optional)", [None]+macros,index=0)

        df_r = df_all.copy()
        if exp_year is not None: df_r = df_r[df_r["year"]==exp_year]
        if exp_state is not None: df_r = df_r[df_r["state_name"]==exp_state]
        if exp_sector is not None: df_r = df_r[df_r["sector_macro"]==exp_sector]

        if not df_r.empty:
            rep = df_r.groupby(["year","state_name","sector_macro"],dropna=True).agg({"injuries":"sum","fatalities":"sum","hoursworked":"sum"}).reset_index()
            rep["TRIR (/200k hrs)"] = (rep["injuries"]/rep["hoursworked"]).fillna(0)*200000
            rep = rep.rename(columns={"year":"Year","state_name":"State","sector_macro":"Macro Sector","injuries":"Injuries","fatalities":"Fatalities"}).drop(columns=["hoursworked"])

            buffer_xlsx = io.BytesIO()
            with pd.ExcelWriter(buffer_xlsx,engine="xlsxwriter") as writer:
                rep.to_excel(writer,index=False,sheet_name="HSE Report")
            st.download_button("⬇️ Download Excel",buffer_xlsx.getvalue(),"hse_report.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            ...
            elements.append(Paragraph(filt,styles["Normal"])); elements.append(Spacer(1,8))
            ...
            st.download_button("⬇️ Download PDF",buffer_pdf.getvalue(),"hse_report.pdf","application/pdf")